        ...
        <% loop $profile %>
            <div class="video-details__container">
                <div class="video-details__poster-wrapper">
                    <div class="video-details__poster" style="background-image:url('{$Poster.ScaleWidth(350).Link}'), url('{$resourceURL('themes/app/images/blank.png')}')">
                    </div>
                ...
                <div class="video-details__metadata">
                   ...
                   <% loop $Up.metadata %>
                        <p class="video-details__ratings">
                            ...
                            $Runtime |
                            <span title="IMDB rating">$imdbRating <i class="fa fa-star rating" aria-hidden="true"></i></span>
                        </p>
                        ...
                        <div class="video-details__credits">
                            ...
                            <% if $Top.seasonLinks %>
                                ...
                                <% loop $Top.seasonLinks %>
                                    $seasons.raw <% if not $Last %>|<% end_if %>
                                <% end_loop %>
                            <% end_if %>
                        </div>
                   <% end_loop %>
                    ...
                    <div class="video-details__keywords">
                        <% if $genres != 0 %>$genres.raw <% end_if %>
                        <% if $keywords != 0 %>$keywords.raw <% end_if %>
                    </div>
                                        ...
                                        <% loop $Up.relatedTitles %>
                                            <a href="{$Up.ProfileURL}title/$ID" title="Browse {$Title} (Updated {$LastEdited.Ago})">
                                                <div
                                                        ...
                                                        style="background-image:url('{$Top.getPosterImageByID($PosterID).link}'),url('{$resourceURL('themes/app/images/blank.png')}')"
                                                ></div>
                                            </a>
                                        <% end_loop %>
                                    </div>
